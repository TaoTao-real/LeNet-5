		...
		for (int y = 0; y < height_image_C5_CNN; y++) {
			for (int x = 0; x < width_image_C5_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_S4_CNN; inc++) {
					...
					__m256 m256_delta_neuron_C5 = _mm256_set1_ps(delta_neuron_C5[index]);
					for(int wy = 0; wy < height_kernel_conv_CNN; wy++){
						int addr3 = addr1 + wy*width_kernel_conv_CNN;
						int addr4 = addr2 + wy*width_image_S4_CNN;
						int addr5 = outc;
						__m256 add_m256_delta_neuron_S4;
						__m256 m256_weight_C5 = _mm256_loadu_ps(weight_C5 + addr3);
						__m256 add_m256_delta_weight_C5;
						__m256 m256_neuron_S4 = _mm256_loadu_ps(neuron_S4 + addr4);
						__m256 add_m256_delta_bias_C5;

						add_m256_delta_neuron_S4 = _mm256_mul_ps(_mm256_mul_ps(m256_delta_neuron_C5, m256_weight_C5),m256_activation_function_tanh_derivative(m256_neuron_S4));
						add_m256_delta_weight_C5 = _mm256_mul_ps(m256_delta_neuron_C5, m256_neuron_S4);
						add_m256_delta_bias_C5 = m256_delta_neuron_C5;
						
						float temp_delta_neuron_S4[width_kernel_conv_CNN];
						_mm256_storeu_ps(temp_delta_neuron_S4, add_m256_delta_neuron_S4);
						float temp_delta_weight_C5[width_kernel_conv_CNN];
						_mm256_storeu_ps(temp_delta_weight_C5, add_m256_delta_weight_C5);
						float temp_delta_bias_C5[width_kernel_conv_CNN];
						_mm256_storeu_ps(temp_delta_bias_C5, add_m256_delta_bias_C5);
						for(int i = 0; i < 5; ++i){
							delta_neuron_S4[addr4 + i] += temp_delta_neuron_S4[i];
							delta_weight_C5[addr3 + i] += temp_delta_weight_C5[i];
							delta_bias_C5[addr5 + i] += temp_delta_bias_C5[i];
						}
					}
				}
			} //index
		...
		for (int y = 0; y < height_image_C3_CNN; y++) {
			for (int x = 0; x < width_image_C3_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_S2_CNN; inc++) {
					...
					__m256 m256_delta_neuron_C3 = _mm256_set1_ps(delta_neuron_C3[index]);
					for(int wy = 0; wy < height_kernel_conv_CNN; wy++){
						int addr3 = addr1 + wy*width_kernel_conv_CNN;
						int addr4 = addr2 + wy*width_image_S2_CNN;
						int addr5 = outc;
						__m256 add_m256_delta_neuron_S2;
						__m256 m256_weight_C3 = _mm256_loadu_ps(weight_C3 + addr3);
						__m256 add_m256_delta_weight_C3;
						__m256 m256_neuron_S2 = _mm256_loadu_ps(neuron_S2 + addr4);
						__m256 add_m256_delta_bias_C3;

						add_m256_delta_neuron_S2 = _mm256_mul_ps(_mm256_mul_ps(m256_delta_neuron_C3, m256_weight_C3),m256_activation_function_tanh_derivative(m256_neuron_S2));
						add_m256_delta_weight_C3 = _mm256_mul_ps(m256_delta_neuron_C3, m256_neuron_S2);
						add_m256_delta_bias_C3 = m256_delta_neuron_C3;
						
						float temp_delta_neuron_S2[width_kernel_conv_CNN];
						_mm256_storeu_ps(temp_delta_neuron_S2, add_m256_delta_neuron_S2);
						float temp_delta_weight_C3[width_kernel_conv_CNN];
						_mm256_storeu_ps(temp_delta_weight_C3, add_m256_delta_weight_C3);
						float temp_delta_bias_C3[width_kernel_conv_CNN];
						_mm256_storeu_ps(temp_delta_bias_C3, add_m256_delta_bias_C3);
						for(int i = 0; i < 5; ++i){
							delta_neuron_S2[addr4 + i] += temp_delta_neuron_S2[i];
							delta_weight_C3[addr3 + i] += temp_delta_weight_C3[i];
							delta_bias_C3[addr5 + i] += temp_delta_bias_C3[i];
						}
					}
				}
			} //index
		...
		for (int y = 0; y < height_image_C1_CNN; y++) {
			for (int x = 0; x < width_image_C1_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_input_CNN; inc++) {
					...
					__m256 m256_delta_neuron_C1 = _mm256_set1_ps(delta_neuron_C1[index]);
					for(int wy = 0; wy < height_kernel_conv_CNN; wy++){
						int addr3 = addr1 + wy*width_kernel_conv_CNN;
						int addr4 = addr2 + wy*width_image_S4_CNN;
						int addr5 = outc;
						__m256 add_m256_delta_neuron_input;
						__m256 m256_weight_C1 = _mm256_loadu_ps(weight_C1 + addr3);
						__m256 add_m256_delta_weight_C1;
						__m256 m256_data_single_image = _mm256_loadu_ps(data_single_image + addr4);
						__m256 add_m256_delta_bias_C1;

						add_m256_delta_neuron_input = _mm256_mul_ps(_mm256_mul_ps(m256_delta_neuron_C1, m256_weight_C1),m256_activation_function_tanh_derivative(m256_data_single_image));
						add_m256_delta_weight_C1 = _mm256_mul_ps(m256_delta_neuron_C1, m256_data_single_image);
						add_m256_delta_bias_C1 = m256_delta_neuron_C1;
						
						float temp_delta_neuron_input[width_kernel_conv_CNN];
						_mm256_storeu_ps(temp_delta_neuron_input, add_m256_delta_neuron_input);
						float temp_delta_weight_C1[width_kernel_conv_CNN];
						_mm256_storeu_ps(temp_delta_weight_C1, add_m256_delta_weight_C1);
						float temp_delta_bias_C1[width_kernel_conv_CNN];
						_mm256_storeu_ps(temp_delta_bias_C1, add_m256_delta_bias_C1);
						for(int i = 0; i < 5; ++i){
							delta_neuron_input[addr4 + i] += temp_delta_neuron_input[i];
							delta_weight_C1[addr3 + i] += temp_delta_weight_C1[i];
							delta_bias_C1[addr5 + i] += temp_delta_bias_C1[i];
						}
					}
				}
			} //index
