...
bool CNN::Backward_output()
{
        ...
        for (int i = 0; i < num_neuron_output_CNN; i+=5) {
				...
				*/
				...
				init_variable(dy_da, 0.0, num_neuron_output_CNN);
                dy_da[i + 1] = activation_function_tanh_derivative(neuron_output[i + 1]);
                delta_neuron_output[i + 1] = dot_product(dE_dy, dy_da, num_neuron_output_CNN);

				init_variable(dy_da, 0.0, num_neuron_output_CNN);
                dy_da[i + 2] = activation_function_tanh_derivative(neuron_output[i + 2]);
                delta_neuron_output[i + 2] = dot_product(dE_dy, dy_da, num_neuron_output_CNN);

				init_variable(dy_da, 0.0, num_neuron_output_CNN);
                dy_da[i + 3] = activation_function_tanh_derivative(neuron_output[i + 3]);
                delta_neuron_output[i + 3] = dot_product(dE_dy, dy_da, num_neuron_output_CNN);

				init_variable(dy_da, 0.0, num_neuron_output_CNN);
                dy_da[i + 4] = activation_function_tanh_derivative(neuron_output[i + 4]);
                delta_neuron_output[i + 4] = dot_product(dE_dy, dy_da, num_neuron_output_CNN);
        }
        return true;
}
...
bool CNN::Backward_C5()
{
	...
	for (int j = 0; j < num_neuron_output_CNN; j++) {
		for (int k = 0; k < num_neuron_C5_CNN; k+=5) {
			...
			*/
			int addr10 = k * num_neuron_output_CNN + j;    //当前权重
			int addr20 = j;
			delta_neuron_C5[k] += delta_neuron_output[j] * weight_output[addr10]
								  * activation_function_tanh_derivative(neuron_C5[k]);
			delta_weight_output[addr10] += delta_neuron_output[j] * neuron_C5[k];
			delta_bias_output[addr20] += delta_neuron_output[j];

			int addr11 = (k + 1) * num_neuron_output_CNN + j;    //当前权重
			int addr21 = j;
			delta_neuron_C5[k + 1] += delta_neuron_output[j] * weight_output[addr11]
								  * activation_function_tanh_derivative(neuron_C5[k + 1]);
			delta_weight_output[addr11] += delta_neuron_output[j] * neuron_C5[k + 1];
			delta_bias_output[addr21] += delta_neuron_output[j];

			int addr12 = (k + 2) * num_neuron_output_CNN + j;    //当前权重
			int addr22 = j;
			delta_neuron_C5[k + 2] += delta_neuron_output[j] * weight_output[addr12]
								  * activation_function_tanh_derivative(neuron_C5[k + 2]);
			delta_weight_output[addr12] += delta_neuron_output[j] * neuron_C5[k + 2];
			delta_bias_output[addr22] += delta_neuron_output[j];

			int addr13 = (k + 3) * num_neuron_output_CNN + j;    //当前权重
			int addr23 = j;
			delta_neuron_C5[k + 3] += delta_neuron_output[j] * weight_output[addr13]
								  * activation_function_tanh_derivative(neuron_C5[k + 3]);
			delta_weight_output[addr13] += delta_neuron_output[j] * neuron_C5[k + 3];
			delta_bias_output[addr23] += delta_neuron_output[j];

			int addr14 = (k + 4) * num_neuron_output_CNN + j;    //当前权重
			int addr24 = j;
			delta_neuron_C5[k + 4] += delta_neuron_output[j] * weight_output[addr14]
								  * activation_function_tanh_derivative(neuron_C5[k + 4]);
			delta_weight_output[addr14] += delta_neuron_output[j] * neuron_C5[k + 4];
			delta_bias_output[addr24] += delta_neuron_output[j];
			...
			for (int x = 0; x < width_image_C5_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_S4_CNN; inc++) {
					...
					for (int wy = 0; wy < height_kernel_conv_CNN; wy++) {
						...
						int addr3 = addr1 + wy*width_kernel_conv_CNN;  //卷积核索引 W_kj
						int addr4 = addr2 + wy*width_image_S4_CNN;     //S4中的像素索引 S4 k
						int addr5 = outc;
						...
						delta_neuron_S4[addr4 + 1] += delta_neuron_C5[index] * weight_C5[addr3 + 1]
												* activation_function_tanh_derivative(neuron_S4[addr4]);
						delta_weight_C5[addr3 + 1] += delta_neuron_C5[index] * neuron_S4[addr4 + 1];
						delta_bias_C5[addr5] += delta_neuron_C5[index];

						delta_neuron_S4[addr4 + 2] += delta_neuron_C5[index] * weight_C5[addr3 + 2]
												* activation_function_tanh_derivative(neuron_S4[addr4 + 2]);
						delta_weight_C5[addr3 + 2] += delta_neuron_C5[index] * neuron_S4[addr4 + 2];
						delta_bias_C5[addr5] += delta_neuron_C5[index];

						delta_neuron_S4[addr4 + 3] += delta_neuron_C5[index] * weight_C5[addr3 + 3]
												* activation_function_tanh_derivative(neuron_S4[addr4 + 3]);
						delta_weight_C5[addr3 + 3] += delta_neuron_C5[index] * neuron_S4[addr4 + 3];
						delta_bias_C5[addr5] += delta_neuron_C5[index];

						delta_neuron_S4[addr4 + 4] += delta_neuron_C5[index] * weight_C5[addr3 + 4]
												* activation_function_tanh_derivative(neuron_S4[addr4 + 4]);
						delta_weight_C5[addr3 + 4] += delta_neuron_C5[index] * neuron_S4[addr4 + 4];
						delta_bias_C5[addr5] += delta_neuron_C5[index];
						/*
			...
			for (int x = 0; x < width_image_C3_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_S2_CNN; inc++) {
					...
					for (int wy = 0; wy < height_kernel_conv_CNN; wy++) {
						...
						int addr3 = addr1 + wy*width_kernel_conv_CNN;  //卷积核索引 W_kj
						int addr4 = addr2 + wy*width_image_S2_CNN;     //S2中的像素索引 S2 k
						int addr5 = outc;
						...
						delta_neuron_S2[addr4 + 1] += delta_neuron_C3[index] * weight_C3[addr3 + 1]
												* activation_function_tanh_derivative(neuron_S2[addr4 + 1]);
						delta_weight_C3[addr3 + 1] += delta_neuron_C3[index] * neuron_S2[addr4 + 1];
						delta_bias_C3[addr5] += delta_neuron_C3[index];	

						delta_neuron_S2[addr4 + 2] += delta_neuron_C3[index] * weight_C3[addr3 + 2]
												* activation_function_tanh_derivative(neuron_S2[addr4 + 2]);
						delta_weight_C3[addr3 + 2] += delta_neuron_C3[index] * neuron_S2[addr4 + 2];
						delta_bias_C3[addr5] += delta_neuron_C3[index];	

						delta_neuron_S2[addr4 + 3] += delta_neuron_C3[index] * weight_C3[addr3 + 3]
												* activation_function_tanh_derivative(neuron_S2[addr4 + 3]);
						delta_weight_C3[addr3 + 3] += delta_neuron_C3[index] * neuron_S2[addr4 + 3];
						delta_bias_C3[addr5] += delta_neuron_C3[index];	

						delta_neuron_S2[addr4 + 4] += delta_neuron_C3[index] * weight_C3[addr3 + 4]
												* activation_function_tanh_derivative(neuron_S2[addr4 + 4]);
						delta_weight_C3[addr3 + 4] += delta_neuron_C3[index] * neuron_S2[addr4 + 4];
						delta_bias_C3[addr5] += delta_neuron_C3[index];	
						/*
						for (int wx = 0; wx < width_kernel_conv_CNN; wx++) {
                            int addr3 = addr1 + wy*width_kernel_conv_CNN + wx;  //卷积核索引 W_kj
			...
			for (int x = 0; x < width_image_C1_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_input_CNN; inc++) {
					...
					for (int wy = 0; wy < height_kernel_conv_CNN; wy++) {
						...
						int addr3 = addr1 + wy*width_kernel_conv_CNN;  //卷积核索引 W_kj
						int addr4 = addr2 + wy*width_image_input_CNN;     //input中的像素索引 input k
						int addr5 = outc;
						...
						delta_neuron_input[addr4 + 1] += delta_neuron_C1[index] * weight_C1[addr3 + 1]
											* activation_function_tanh_derivative(data_single_image[addr4 +1]);
						delta_weight_C1[addr3 + 1] += delta_neuron_C1[index] * data_single_image[addr4 + 1];
						delta_bias_C1[addr5] += delta_neuron_C1[index];

						delta_neuron_input[addr4 + 2] += delta_neuron_C1[index] * weight_C1[addr3 + 2]
											* activation_function_tanh_derivative(data_single_image[addr4 + 2]);
						delta_weight_C1[addr3 + 2] += delta_neuron_C1[index] * data_single_image[addr4 + 2];
						delta_bias_C1[addr5] += delta_neuron_C1[index];

						delta_neuron_input[addr4 + 3] += delta_neuron_C1[index] * weight_C1[addr3 + 3]
											* activation_function_tanh_derivative(data_single_image[addr4 + 3]);
						delta_weight_C1[addr3 + 3] += delta_neuron_C1[index] * data_single_image[addr4 + 3];
						delta_bias_C1[addr5] += delta_neuron_C1[index];

						delta_neuron_input[addr4 + 4] += delta_neuron_C1[index] * weight_C1[addr3 + 4]
											* activation_function_tanh_derivative(data_single_image[addr4 + 4]);
						delta_weight_C1[addr3 + 4] += delta_neuron_C1[index] * data_single_image[addr4 + 4];
						delta_bias_C1[addr5] += delta_neuron_C1[index];
						/*
						for (int wx = 0; wx < width_kernel_conv_CNN; wx++) {
                            int addr3 = addr1 + wy*width_kernel_conv_CNN + wx;  //卷积核索引 W_kj
