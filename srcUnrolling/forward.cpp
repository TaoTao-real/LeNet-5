			...
			for (int x = 0; x < width_image_C1_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_input_CNN; inc++) {
					...
					for (int wy = 0; wy < height_kernel_conv_CNN; wy++) {
						sum += *ppw++ * ppi[wy * width_image_input_CNN + 0];
						sum += *ppw++ * ppi[wy * width_image_input_CNN + 1];
						sum += *ppw++ * ppi[wy * width_image_input_CNN + 2];
						sum += *ppw++ * ppi[wy * width_image_input_CNN + 3];
						sum += *ppw++ * ppi[wy * width_image_input_CNN + 4];
		...
		for (int y=0; y<height_image_S2_CNN; y++) {
			for (int x=0; x<width_image_S2_CNN; x++) {
				...
				for (int m = 0; m < width_kernel_pooling_CNN; m++) {
					/*
					neuron_S2[index] += weight_S2[i] * neuron_C1[(rows + m) * width_image_C1_CNN + cols + 0 + block];
					neuron_S2[index] += weight_S2[i] * neuron_C1[(rows + m) * width_image_C1_CNN + cols + 1 + block];
					*/
			...
			for (int x = 0; x < width_image_C3_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_S2_CNN; inc++) {
					...
					for (int wy = 0; wy < height_kernel_conv_CNN; wy++) {
						...
						sum += *ppw++ * ppi[wy * width_image_S2_CNN + 0];
	...
	for (int i=0; i<num_map_S4_CNN; i++) {
		...
		for (int y=0; y<height_image_S4_CNN; y++) {
			for (int x=0; x<width_image_S4_CNN; x++) {
				...
				for (int m = 0; m < width_kernel_pooling_CNN; m++) {
					/*
					neuron_S4[index] += weight_S4[i] * neuron_C3[(rows + m) * width_image_C3_CNN + cols + 0 + block];
					neuron_S4[index] += weight_S4[i] * neuron_C3[(rows + m) * width_image_C3_CNN + cols + 1 + block];
					*/
					for (int n = 0; n < height_kernel_pooling_CNN; n++) {
			...
			for (int x = 0; x < width_image_C5_CNN; x++) {
				...
				for (int inc = 0; inc < num_map_S4_CNN; inc++) {
					...
					for (int wy = 0; wy < height_kernel_conv_CNN; wy++) {
						...
						sum += *ppw++ * ppi[wy * width_image_S4_CNN + 0];
						sum += *ppw++ * ppi[wy * width_image_S4_CNN + 1];
						sum += *ppw++ * ppi[wy * width_image_S4_CNN + 2];
						sum += *ppw++ * ppi[wy * width_image_S4_CNN + 3];
						sum += *ppw++ * ppi[wy * width_image_S4_CNN + 4];
						/*
						for (int wx = 0; wx < width_kernel_conv_CNN; wx++) {
...
bool CNN::Forward_output()
{
	for (int i = 0; i < num_neuron_output_CNN; i++) {
		neuron_output[i] = 0.0;
		for (int c = 0; c < num_neuron_C5_CNN; c+=4) {
			
			neuron_output[i] += weight_output[c * num_neuron_output_CNN + i] * neuron_C5[c];
			neuron_output[i] += weight_output[(c + 1) * num_neuron_output_CNN + i] * neuron_C5[c + 1];
			neuron_output[i] += weight_output[(c + 2) * num_neuron_output_CNN + i] * neuron_C5[c + 2];
			neuron_output[i] += weight_output[(c + 3) * num_neuron_output_CNN + i] * neuron_C5[c + 3];
			//neuron_output[i] += weight_output[c * num_neuron_output_CNN + i] * neuron_C5[c];
		}
